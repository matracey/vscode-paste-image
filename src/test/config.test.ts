...
suite("Extension Test Suite (config)", () => {
  ...
  test("getPasteTemplate test", async () => {
    assert.equal(
      "![](${relativePath})",
      PasterConfig.getPasteTemplate("markdown")
    );
    assert.equal(
      "image::${relativePath}[]",
      PasterConfig.getPasteTemplate("asciidoc")
    );
    assert.equal("${relativePath}", PasterConfig.getPasteTemplate("html"));
  });

  test("getBasePath test relative", async () => {
  ...
  test("getBasePath test absolute", async () => {
    const uri: vscode.Uri = vscode.Uri.joinPath(rootDir, "zzz/sample.abc");
    const actual: vscode.Uri = vscode.Uri.joinPath(
      uri,
      "../../../../../../../../../../../../../../../../../../../../../",
      "a/b/c"
    );
  ...
  test("getBasePath test workspaceRoot", async () => {
    const uri: vscode.Uri = vscode.Uri.joinPath(rootDir, "zzz/sample.abc");
    await conf.update("path", "${workspaceRoot}");
    assert.equal("" + rootDir, "" + PasterConfig.getBasePath(uri));
  });

  test("getBasePath test projectRoot", async () => {
    const uri: vscode.Uri = vscode.Uri.joinPath(rootDir, "zzz/sample.abc");
    await conf.update("path", "${projectRoot}");
    assert.equal("" + rootDir, "" + PasterConfig.getBasePath(uri));
  });

  test("getBasePath test currentFileDir", async () => {
    const uri: vscode.Uri = vscode.Uri.joinPath(rootDir, "zzz/sample.abc");
    await conf.update("path", "${currentFileDir}");
    assert.equal("" + uri, "" + PasterConfig.getBasePath(uri));
  });

  test("getBasePath test fileBasenameNoExtension", async () => {
    const uri: vscode.Uri = vscode.Uri.joinPath(rootDir, "zzz/sample.abc");
    await conf.update("path", "${fileBasenameNoExtension}");
    assert.equal(
      "" + rootDir + "/zzz/sample",
      "" + PasterConfig.getBasePath(uri)
    );
  });

  test("getBasePath test fileBasename", async () => {
    const uri: vscode.Uri = vscode.Uri.joinPath(rootDir, "zzz/sample.abc");
    await conf.update("path", "${fileBasename}");
    assert.equal(
      "" + rootDir + "/zzz/sample.abc",
      "" + PasterConfig.getBasePath(uri)
    );
  });

  test("getBasePath test fileDirname", async () => {
    const uri: vscode.Uri = vscode.Uri.joinPath(rootDir, "zzz/sample.abc");
  ...
  test("getBasePath test projectRoot fileBasenameNoExtension", async () => {
    const uri: vscode.Uri = vscode.Uri.joinPath(rootDir, "zzz/sample.abc");
