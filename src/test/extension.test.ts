...
suite("Extension Test Suite (command)", () => {
  ...
  test("command vscode-paste-image.pasteImage test", async () => {
    ...
    const editor = await vscode.window.showTextDocument(fpath, {
      preview: false,
    });
  ...
  test("command vscode-paste-image.pasteBase64Image test", async () => {
    const fpath = vscode.Uri.joinPath(rootDir, "sample.md");
    const buff = new TextEncoder().encode("sample");
    await vscode.workspace.fs.writeFile(fpath, buff);
    const editor = await vscode.window.showTextDocument(fpath, {
  ...
  test("command vscode-paste-image.createImage test", async () => {
